...
public class HttpUtils {
    ...
    public static void outLogin(Context context, String responseMsg) {
        ...
        CacheUtils.INSTANCE.setString(context, USER_LOGIN_TOKEN, "");
    }
...
//        params.put("remark", RxDeviceTool.getBuildMANUFACTURER() + "  " + RxDeviceTool.getBuildBrandModel());
        params.put("devicetoken", CacheUtils.INSTANCE.getString(context, USER_DEVICE_TOKEN, ""));
...
//        String registrationId = MainApplication.Companion.getPushAgent().getRegistrationId();
        String cacheDeviceToken = CacheUtils.INSTANCE.getString(context, USER_DEVICE_TOKEN, "");
        String logintoken  = CacheUtils.INSTANCE.getString(context, USER_LOGIN_TOKEN, "");
        Log.e("ooooooooooo",""+cacheDeviceToken);
        Log.e("ooooooooooo",""+logintoken);
        header.put("devicetoken", cacheDeviceToken == null ? registrationId : cacheDeviceToken);
        header.put("logintoken", logintoken == null ? registrationId : cacheDeviceToken);
    ...
    private static class SaveDataRunaable implements Runnable {
        ...
        private void setResponseHeaders(Context context, Headers headers) {
            ...
            if (headers.get("X-Device-Token") != null) {
                if (headers.get("X-Device-Token").equals("0")) {
                    outLogin(context, "");
                } else {
                    if (headers.get("X-Device-Token") != null && headers.get("X-Device-Token") != "") {//判断服务器是否有返回cookie
                        CacheUtils.INSTANCE.setString(context, USER_DEVICE_TOKEN, headers.get("X-Device-Token"));
                        SaveMessage.Savedevicetoken(headers.get("X-Device-Token"),context);
                    }
                    if (headers.get("X-Login-Token") != null && headers.get("X-Login-Token") != "") {//判断服务器是否有返回cookie
                        CacheUtils.INSTANCE.setString(context, USER_LOGIN_TOKEN, headers.get("X-Login-Token"));
                        SaveMessage.Savelogintoken(headers.get("X-Login-Token"),context);
                    }
//                    Log.e("ppppppppp",""+headers.get("X-Device-Token"));
//                    Log.e("ppppppppp",""+headers.get("X-Login-Token"));
                    CacheUtils.INSTANCE.setBoolean(context, Constants.User.IS_LOGIN, true);
                }
            }
        ...
        private void setResponseBody(String body) {
            ...
            switch (code) {
                case 200:
                    ...
                    break;
                case 400:
                    CacheUtils.INSTANCE.setString(context, "logintoken ", "");
...
//                    UpDataApkTools.getInstance().setContext(context).upDataApkData(false, string);
                    break;
                case 403:
                    NetWork.INSTANCE.getCom( context, new NetWork.TokenCallBack() {
                        @Override
                        public void doNext(String resultData, Headers headers) {
                            Log.e("",""+resultData);
//                            CacheUtils.INSTANCE.setString(context, "devicetoken", "");
                            getToken(context, doNext);
                        }
                        @Override
                        public void doError(String msg) {
