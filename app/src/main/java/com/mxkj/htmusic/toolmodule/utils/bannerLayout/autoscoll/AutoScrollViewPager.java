package com.mxkj.htmusic.toolmodule.utils.bannerLayout.autoscoll;
 ...
 * 使用的trinea的开源代码，解决嵌套其他带滚动父类里面时不能自动滚动问题
 * <ul>
...
public class AutoScrollViewPager extends ViewPager {
     ...
     * <li>if event is drop_down, stop auto scroll.</li>
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (stopScrollWhenTouch) {
            if (ev.getAction() == MotionEvent.ACTION_DOWN && isAutoScroll) {
                ...
                startAutoScroll();
            } else if (ev.getAction() == MotionEvent.ACTION_CANCEL && isStopByTouch){//解决嵌套其他带滚动父类里面时不能自动滚动
                startAutoScroll();
            }
        }
