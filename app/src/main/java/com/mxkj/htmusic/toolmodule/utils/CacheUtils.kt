...
object CacheUtils {
    ...
    private fun getSp(context: Context, key: String?): SharedPreferences? {
        sp = if (key == null || key == Constants.User.IS_LOGIN || key == Constants.User.USER_JSON || key == Constants.User.USER_LOGIN_TOKEN || key == "token"|| key == "home"|| key == "remark"|| key == "devicetoken") {
