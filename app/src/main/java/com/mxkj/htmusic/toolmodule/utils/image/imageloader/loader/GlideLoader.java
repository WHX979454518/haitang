...
import com.mxkj.htmusic.toolmodule.utils.image.imageloader.config.AnimationMode;
import com.mxkj.htmusic.toolmodule.utils.image.imageloader.config.GlobalConfig;
import com.mxkj.htmusic.toolmodule.utils.image.imageloader.config.PriorityMode;
import com.mxkj.htmusic.toolmodule.utils.image.imageloader.config.ScaleMode;
import com.mxkj.htmusic.toolmodule.utils.image.imageloader.config.ShapeMode;
...
public class GlideLoader implements ILoader {
    ...
    private DrawableTypeRequest getDrawableTypeRequest(SingleConfig config, RequestManager requestManager) {
        ...
        if (!TextUtils.isEmpty(config.getUrl())) {
            ...
            GlideUrl glideUrl = new GlideUrl(ImageUtil.appendUrl(config.getUrl()), new LazyHeaders.Builder()
                    .addHeader("Accept","image/webp,image/apng,image/*,*/*;q=0.8")
                    .addHeader("Accept-Encoding","gzip, deflate, br")
                    .addHeader("Accept-Language","zh-CN,zh;q=0.9")
                    .addHeader("User-Agent",new GetUserAgent().getUserAgent())
                    .build());
            request = requestManager.load(glideUrl);
