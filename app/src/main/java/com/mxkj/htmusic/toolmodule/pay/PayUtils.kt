...
object PayUtils {
    ...
    fun pay(context: Context, project_id: Int, project_task_id: Int,contract_id: Int, uid: Int,musician_id: Int, pay_type: Int,order_money: String) {
...
//        params.put("user_coupon_id", user_coupon_id )
        NetWork.getcreateorder(context, params, object : NetWork.TokenCallBack {
            override fun doNext(resultData: String, headers: Headers?) {
                val jObj = JSON.parseObject(resultData)
                val data = jObj.getJSONObject("data")
                val order_sn = data.getString("order_sn")
//                val payurl = data.getString("pay_url")
//                val pay_type = data.getString("pay_type")

                CacheUtils.setString(MainApplication.application, "giftStr", "")
                if(pay_type==2){
                    useAliPay(context, resultData)
                }else if(pay_type==1){
                    useWxPay(context, resultData)
                }else if(pay_type==3){
                    useQQPay(context, resultData)
                }else if(pay_type==4){
                    useQQPay(context, resultData)
                }

//                getOrderInfo(context, ordercode, payurl, payment_type)
            }
