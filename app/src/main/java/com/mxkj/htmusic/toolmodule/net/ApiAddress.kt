...
object ApiAddress {
    const val BASE_URL = "http://api.demo.com/api"
    const val BASE_URLTWO = "http://api.demo.com"
    const val GET_TOKEN = "/com/auth"//获取授权接口
    const val GET_CODE = "/code/send_sms"//获取验证码
    const val FAST_LOGIN = "/user/fast_login"//快速登录
    const val READ = "/member.user/read"//获取我的用户信息
    const val FAST_SET_PASSWORD = "/user/fast_set_password"//快速登录设置密码
    const val GET_PWD_LOGIN = "/user/login"//账号密码登录

    //首页模块
    const val GET_PROJECT_INDEX = "/project/index"//首页项目列表
    const val GET_PROJECT_CONDITION = "/project/condition"//项目列表条件
    const val GET_PROJECT_INFO = "/project/info"//项目详情
    const val GET_PROJECT_TASK_INFO = "/project/task_info"///获取任务简介
    const val GET_PROJECT_COLLECT = "/member.project/collect"//收藏项目
    const val GET_PROJECT_UN_COLLECT = "/member.project/un_collect"//取消收藏项目


    const val GET_PROJECT_ENROLL = "/project-enroll"//音乐人项目报名



    const val GET_ADD_PROJECT = "/add-project"//发布项目


    const val GET_WORK_STYTLE = "/work-style"//作品分格
    const val GET_USE_SCOPE = "/get-use-scope"//获取作品用途(使用范围)
    const val GET_USE_CHANNEL = "/get-use-channel"//获取使用渠道
    const val GET_WORK_TYPE = "/work-type"//获取工种类型
    const val GET_ALL_TASK = "/project/get_all_task"//获取项目的所有任务/工种

    const val GET_ENROOL = "/get-enroll"//获取应征列表之报名音乐人列表
    const val GET_CONREACT = "/get-contract"//获取应征列表之选定音乐人列表
    const val GET_CONTRACT = "/contract"//招募人发送合同给音乐人
    const val GET_CONTRACTTEXT = "/read-contract"//获取指定合同信息

    const val GET_CONTRACT_LOG = "/get-contract-log"//获取合作状态纪录


    const val GET_PROJECT = "/get-project"// 招募人获取 '我的项目' 列表


    const val GET_MUSICAN_CONTRACT = "/get-musician-contract"//音乐人获取'我参与的' 项目列表
    const val GET_REJECT_SIGN = "/reject-sign"//音乐人拒绝签约
    const val GET_AGREE_SIGN = "/agree-sign"//音乐人同意签约
    const val GET_APPLY_STOP = "/apply-stop"//音乐人申请终止项目合同
    const val GET_APPLY_DELAY = "/apply-delay"//音乐人申请延长项目合同周期



    const val GET_CANCEL_SIGN = "/cancel-sign"//招募人取消签约
    const val GET_CREATE_ORDER = "/create-order"//招募人支付
    //进行中
    const val GET_FAST_CHECK = "/fast-check"//招募人快速验收
    const val GET_DELAY = "/delay"//招募人延长项目合同周期
    const val GET_CONRACT_STOP = "/contract-stop"//招募人终止合作

    const val GET_EVAlUATE = "/evaluate"//合作完成相互评价
...
//    const val MUSIC_DOWN = "/v2/api/music/getDownKey"//音乐下载
    ...
    const val GET_HELP_PROBLEMLIST = "com/get_city"//问题分类类列表
