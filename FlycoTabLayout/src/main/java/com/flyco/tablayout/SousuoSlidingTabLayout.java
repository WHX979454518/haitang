 ...
 */
public class SousuoSlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    public SousuoSlidingTabLayout(Context context) {
        this(context, null, 0);
    }

    public SousuoSlidingTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SousuoSlidingTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        mIndicatorDrawable = ta.getDimension(R.styleable.SlidingTabLayout_tl_tab_padding, mTabSpaceEqual || mTabWidth > 0 ? dp2px(0) : dp2px(20));
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    public void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            ImageView tab_indic =  tabView.findViewById(R.id.tab_indic);
            if (i==0&&mIndicatorHeight==0){
                tab_title.setTextSize(18f);
                tab_indic.setVisibility(VISIBLE);
            }else{
                tab_indic.setVisibility(GONE);
                tab_title.setTextSize(15f);
            }

            if (isSelect&&mIndicatorHeight==0){
                tab_title.setTextSize(18f);
                tab_indic.setVisibility(VISIBLE);
            }else{
                tab_indic.setVisibility(GONE);
...
//            TODO 一般模式下，用Ui给的图片。只要布局文件里tl_indicator_height大于0，就认为需要显示底部指示器
            if (mIndicatorHeight > 0) {
                mIndicatorDrawable.setColor(mIndicatorColor);
